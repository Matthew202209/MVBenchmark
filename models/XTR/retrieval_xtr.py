...
class XtrRetriever(object):
    ...
    def build_index_bruteforce(self, documents, index_dir=None, batch_size=32):
        ...
        class BruteForceSearcher(object):
            def search_batched(self, query_embeds, final_num_neighbors, **kwargs):
                ...
                return top_ids, [q_score[q_top_ids] for q_score, q_top_ids in
                                 zip(scores, top_ids)]  # (Q x top_k, Q x top_k)
    ...
    def build_index_faiss(self, documents, batch_size=32, doc_sample_ratio=0.5, vec_sample_ratio=0.1, seed=29,
                          index_dir=None, code_size=16, nprobe=4):
        ...
        smpl_token_embeds = np.zeros(
            (int(len(documents) * doc_sample_ratio * self.max_seq_len * vec_sample_ratio), self.token_embed_dim),
            dtype=np.float32)
        num_tokens = 0
        ...
        add_count = 0
        add_token_ids = []
        add_token_embeds = np.zeros((int(batch_size * add_size * self.max_seq_len), self.token_embed_dim),
                                    dtype=np.float32)
        all_num_tokens = 0
        for batch_idx in tqdm(range(0, len(documents), batch_size)):
            batch_docs = documents[batch_idx:batch_idx + batch_size]
            batch_embeds, batch_offsets = self.get_flatten_embeddings(batch_docs)
            batch_token_len = [batch_offsets[i + 1] - batch_offsets[i] for i, offset in
                               enumerate(batch_offsets[:-1])] + [len(batch_embeds) - batch_offsets[-1]]
            ...
            batch_token_ids = [did * self.doc_offset + tid for did in range(batch_idx, batch_idx + len(batch_docs)) for
                               tid in range(batch_token_len[did - batch_idx])]
    ...
    def load_index(self, index_dir, code_size, nprobe):
    ...
    def aggregate_scores(self, batch_result, batch_ems, document_top_k):
        ...
        batch_scores = [get_final_score(did2scores, result[0]) for did2scores, result in
                        zip(batch_did2scores, batch_result)]
        batch_ranking = []
        for final_qd_score in batch_scores:
            rank = sorted([(docid, score) for docid, score in final_qd_score.items()], key=lambda x: x[1],
                          reverse=True)[:document_top_k]
            batch_ranking.append([(r[0], idx, r[1]) for idx, r in enumerate(rank)])
