...
class IndexScorer(IndexLoader, CandidateGeneration):
    ...
    def try_load_torch_extensions(cls, use_gpu):
        ...
        #     return
        build_directory = os.path.expanduser("~/.cache/torch_extensions")
    ...
    def lookup_pids(self, passage_ids, return_mask=False):
        return self.embeddings_strided.lookup_pids(passage_ids)
    ...
    def retrieve_vanilla_colbertv2(self, config, Q):
        Q = Q[:, :config.query_maxlen]  # NOTE: Candidate generation uses only the query tokens
        embedding_ids = self.generate_candidates_vanilla_colbertv2(config, Q)
    ...
    def score_pids(self, config, Q, pids, centroid_scores=None):
        """
        ...
        if self.use_gpu:
            ...
            D_packed = IndexScorer.decompress_residuals(
                ...
                self.codec.bucket_weights.to(torch.float32).to("cpu"),
                self.codec.reversed_bit_map.to("cpu"),
                self.codec.decompression_lookup_table.to("cpu"),
                self.embeddings.residuals,
                self.embeddings.codes,
                self.codec.centroids.to(torch.float32).to("cpu"),
                self.codec.dim,
                self.codec.nbits
            )
        ...
        if Q.size(0) == 1:
            res = colbert_score_packed(Q, D_packed, D_mask, config, self.use_gpu), pids
        else:
            D_strided = StridedTensor(D_packed, D_mask, use_gpu=self.use_gpu)
            D_padded, D_lengths = D_strided.as_padded_tensor()
            res = colbert_score(Q, D_padded, D_lengths, config, use_gpu=self.use_gpu), pids
        return res

    def score_pids_vanilla_colbertv2(self, config, Q, pids, k):
        if self.use_gpu:
            D_packed, D_mask = self.lookup_pids(pids)
        else:
            D_packed = IndexScorer.decompress_residuals(
            ...
            D_mask = self.doclens[pids.long()]
        if Q.size(0) == 1:
            return colbert_score_packed(Q, D_packed, D_mask, config, self.use_gpu)
        else:
            D_padded, D_lengths = StridedTensor(D_packed, D_mask, use_gpu=self.use_gpu).as_padded_tensor()
            res = colbert_score(Q, D_padded, D_lengths, config, use_gpu=self.use_gpu)
            return res

    def exhaustive(self, config, Q, k):
        ...
        top_pids = torch.concat(top_pids)
        scores, idxs = top_scores.topk(k)
        pids = top_pids[idxs]
        return scores.numpy(), pids.numpy()

    def vanilla_colbertv2(self, config, Q, k):
        with torch.inference_mode():
            pids = self.retrieve_vanilla_colbertv2(config, Q)
            scores = self.score_pids_vanilla_colbertv2(config, Q, pids, k)
            scores_sorter = scores.sort(descending=True)
            pids, scores = pids[scores_sorter.indices].tolist(), scores_sorter.values.tolist()
